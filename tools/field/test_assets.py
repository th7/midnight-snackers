"""Fetching the field document's public assets: the panel artwork and the four goal April Tags.

These are blobs in FIRST's Onshape document, which answers for them unsigned, so this is the one
part of the pipeline that needs no keys. The tests run against a stand-in client and never touch
the network; `assets.py --fetch` is the real thing.

Two failures are worth the code that prevents them. A name Onshape supplies becomes a filename,
so it must not be able to name a path outside the destination. And an error page saved under a
.png is worse than no file at all: it is there, it is the wrong size, and nothing downstream
would say so until a texture came out as noise.
"""
import json
import os
import tempfile
...
class FakeClient:
    """Answers the two calls assets.py makes, and records what was asked for."""
...
class Listing(unittest.TestCase):
    ...
    def test_the_step_blob_is_listed_but_not_a_texture(self):
        """The document keeps its STEP import as a blob too; it is not one of the images."""
...
class Fetching(unittest.TestCase):
    ...
    def test_a_texture_the_document_no_longer_has_stops_the_run(self):
        """Onshape renaming an image must not leave us three textures and a green run."""
    ...
    def test_something_that_is_not_a_png_is_refused(self):
        """A 401 page or an HTML error saved under a .png would sit there looking like a texture."""
...
class Names(unittest.TestCase):
    """Element names come from the document, and a name becomes a filename."""
    ...
    def test_a_hostile_name_in_the_document_stops_the_run(self):
        """Not just rejected at the edge: a fetch that meets one must not write the others and
        carry on as though the document were sound."""
